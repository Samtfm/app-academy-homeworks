...
def below_average_years
  ...
  Movie.where("score < 5").group(:yr).order("yr DESC").select('yr, COUNT(title) AS bad_movies')
...
def alphabetized_actors
  ...
  Actor.order(:name).limit(10).select(:id, :name)
...
def pulp_fiction_actors
  ...
  Actor.joins(:castings).joins(:movies).where("title = 'Pulp Fiction'").select(:id,:name)
end
